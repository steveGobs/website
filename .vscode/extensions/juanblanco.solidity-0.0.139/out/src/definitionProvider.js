...
class SolidityDefinitionProvider {
    ...
    provideDefinitionForType(document, documentStatements, literal, contracts) {
        if (literal.members.length > 0) {
            ...
            return this.provideDefinitionForContractMember(contracts, (element) => element.body.filter(contractElement => contractElement.name === literal.literal && (contractElement.type === 'StructDeclaration' || contractElement.type === 'EnumDeclaration')));
        }
    }
    /**
